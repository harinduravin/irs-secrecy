...
for r = 1:numrealizations
    ...
    for zz = 1:num_iterations
        ...
        [Patrial, Pbtrial] = power_optimization_iter_box(y,Pmin,Pmax,Hab(:,r),Hac(:,r),Hbc(:,r),sigma_ab,sigma_loop,sigma_c,[w;1]);
    end
end
...
function [Paopt,Pbopt,optimum] = optimizepower(Pmin,Pmax,Hab,Hac,Hbc,sigma_ab,sigma_loop,sigma_c,w)
    ...
    corner4 = sum_secrecy_rate(Hab,Hac,Hbc,Pmax,Pmax,w,sigma_ab,sigma_loop,sigma_c);
    
    % Optimizing between the corner points and the maximum point on Pa + Pb =
    % Pmax line
    [optimum,index]= max([corner1 corner2 corner3 corner4]);
...
function [Pa Pb] = power_optimization_iter_box(y,Pmin,Pmax,Hab,Hac,Hbc,sigma_ab,sigma_loop,sigma_c,w_hat)
