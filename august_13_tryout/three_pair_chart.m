...
for seed = 0:num_seeds
    ...
    for L = 5:5:40
        % Gaussian randomization
        r_vals = sqrt(1 / 2) * (randn(L+1,numSamples) + (1i * randn(L+1,numSamples)));
...
%                     fprintf('reached IRS');
        ...
        %%%%
        if seed_flag == 0
            [min_value, rate_list] = get_min_rate(P_hat,W);
        elseif seed_flag == 1
            min_value = nan;
            seed_flag = 0;
        end
...
function [min_rate, rates] = get_min_rate(power_list, W_val)
    ...
    for i = 1:length(user_list)
        rate_list = [rate_list get_rate(user_list(i), power_list, W_val)];
    end
...
function rate = get_rate(user, power_list, W_val)
    ...
    eves_inf = log2(1 + trace((eves_inf_term2 - eves_inf_term1)*W_val)/(trace(eves_inf_term1*W_val) + sigma_c));

    leg_inf = log2(1 + trace((leg_inf_term2 - leg_inf_term1)*W_val)/(trace(leg_inf_term1*W_val) + sigma_ab + sigma_loop));
